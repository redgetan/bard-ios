...
//        createAlbumIfNotPresent()
    ...
    static func createMissingDirectories() {
        ...
        createStorageDirectory(getPacksStorageDirectory())
    }
    ...
    static func getPacksStorageDirectory() -> String {
        let documentDirectory = NSSearchPathForDirectoriesInDomains(.DocumentDirectory, .UserDomainMask, true)[0]
        return "\(documentDirectory)/packs"
    }

    static func getPackFilePath(packToken: String) -> String {
        return "\(getPacksStorageDirectory())/\(packToken)"
    }
